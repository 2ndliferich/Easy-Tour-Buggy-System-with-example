...
// Shared LocalEvents (moved from tourBuggyEvents.ts so everything packages with components)
...
export type TourStatePayload = { state: 'running' | 'stopped'; targetEntityId: bigint };
export const TourStateEvent = new hz.LocalEvent<TourStatePayload>('TourStateEvent');
...
class TourBuggy extends hz.Component<typeof TourBuggy> {
  ...
  private startMotion() {
    ...
    this.running = true;
    // Announce running state for UI sync
    this.sendLocalBroadcastEvent(TourStateEvent, { state: 'running', targetEntityId: this.entity.id });
  }

  private stopMotion() {
    this.running = false;
    this.kinematicVel = new hz.Vec3(0, 0, 0);
    // Clear world travel override so next start uses initial axis and facing
    this.worldTravelDir = undefined;
    // Announce stopped state for UI sync
    this.sendLocalBroadcastEvent(TourStateEvent, { state: 'stopped', targetEntityId: this.entity.id });
