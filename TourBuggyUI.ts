...
import { TourControlEvent, TourStateEvent } from './TourBuggy';
...
class TourBuggyUI extends ui.UIComponent<typeof TourBuggyUI> {
  ...
  start() {
    ...
    // Listen for buggy state changes to keep UI in sync when stops happen via triggers
    this.connectLocalBroadcastEvent(TourStateEvent, ({ state, targetEntityId }) => {
      const targetBuggy = this.props.buggy;
      if (targetBuggy) {
        // Only sync if this UI is wired to a specific buggy and IDs match
        if (targetEntityId !== targetBuggy.id) return;
        this.running.set(state === 'running');
      } else {
        // No specific target wired: mirror global state.
        // This matches the broadcast semantics used by this UI when toggling without a target.
        this.running.set(state === 'running');
      }
    });
  }
